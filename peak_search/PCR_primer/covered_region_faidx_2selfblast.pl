...
open(IN,"PCR_primer/primer_comp.txt") or die "ERROR::cannot open primer_comp.txt\n";
<IN>; #header
my %primer=();
while(<IN>){
		chomp;
...
foreach my $id (keys %variant){
		if(!defined $primer{$id}{side}){die "$id is not exist in primer\n";}
...
open(OUT,">PCR_primer/all_primer/all_primer_region.fasta");
