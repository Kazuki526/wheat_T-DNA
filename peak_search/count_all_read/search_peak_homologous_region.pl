...
my $peak_file="count_all_read/all_peak_table_with_p.tsv";
my %peak_region=();
my %pvalue=();
...
while(<IN>){
		...
		$peak_region{$line[$col{max_sample}]} .= "$line[$col{chr}]:$line[$col{start}]-$line[$col{end}]\t";
		$pvalue{"$line[$col{chr}]:$line[$col{start}]-$line[$col{end}]"}=$line[$col{max_p}];
}
close IN;

my $fadir="count_all_read/fasta_dir_signif";
...
my $homologdir= "count_all_read/homologous_peak_list_signif";
